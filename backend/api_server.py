...
def melody_search():
    """
    Search for mashup-compatible tracks.
    ...
    try:
        ...
        from melody_matcher import MelodyMatcher
        ...
        # Parse chords to Roman numerals
        converter = ChordConverter()
        progression = converter.convert_progression(chord_input)
        
        # Build filters
        filters = {
            'bpm': data.get('bpm'),
            'bpm_tolerance': data.get('bpm_tolerance', 15),
        ...
        if not suggestions:
            return jsonify({'matches': [], 'message': 'No suggestions from Claude'})
        
        # Match with Tidal and score
        ...
        matches_json = [{
            ...
            'transpose_semitones': m.transpose_semitones,
            'claude_confidence': m.claude_confidence,
            'claude_reason': m.claude_reason
        } for m in matches]
...
def melody_tidal_check_complete():
    """Check if Tidal auth is complete and save session if it is."""
    try:
        client = get_tidal_client()
...
def melody_tidal_disconnect():
    ...
    try:
        ...
        if _tidal_client:
            ...
            _tidal_client = None  # Reset the global client
...
def melody_create_playlist():
    """
    Create Tidal playlist with matched tracks.
    
    Expected JSON body:
    {
        "name": "Mashup Discovery - 2024-12-01",
        "target_key": "A minor",
        "target_bpm": 120,
        "matches": [array of match objects from search]
    }
    """
    try:
        from melody_matcher import MelodyMatcher, MelodyMatch
        
        data = request.json
        playlist_name = data.get('name', 'Mashup Discovery')
        target_key = data.get('target_key', 'C major')
        ...
        # Create playlist
        matcher = MelodyMatcher()
        playlist_url = matcher.create_playlist_with_guide(
            matches,
            playlist_name,
            target_key,
            target_bpm
